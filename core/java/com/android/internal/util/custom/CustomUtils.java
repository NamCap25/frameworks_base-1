...
public class CustomUtils {
    ...
    public static float getLowerResolutionScale(Resources res, boolean checkEnabled) {
        float scale = 1f;
        String scale_str = res.getString(R.string.config_lower_resolution_scale);
        try {
            scale = Float.parseFloat(scale_str);
            if (checkEnabled && SystemProperties.getInt("persist.sys.display.lower_resolution", 0) == 0) scale = 1f;
    ...
    public static int getQSRowsCount(Context context) {
        if (context.getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) {
            return Settings.System.getIntForUser(
                    context.getContentResolver(), "qs_layout_rows",
                    4, UserHandle.USER_CURRENT);
        }
        return Settings.System.getIntForUser(
                context.getContentResolver(), "qs_layout_rows_landscape",
                2, UserHandle.USER_CURRENT);
    }

    public static int getQuickQSRowsCount(Context context) {
        if (context.getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) {
            return Settings.System.getIntForUser(
                    context.getContentResolver(), "qqs_layout_rows",
                    2, UserHandle.USER_CURRENT);
        }
        return 1;
    }
